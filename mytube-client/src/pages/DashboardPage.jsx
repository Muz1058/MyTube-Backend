...
const DashboardPage = () => {
  ...
  const handleTogglePublishClick = async (videoId) => {
    try {
      ...
      setVideos((prev) =>
        prev.map((v) => (v._id === videoId ? { ...v, isPublished: response.isPublished } : v))
      );
      toast.success(
        response.isPublished ? "Video published!" : "Video set to private.",
      );
    } catch (err) {
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      setVideos((prev) =>
        prev.map((v) =>
          v._id === editingVideo._id ? { ...v, ...updated } : v,
        ),
      );
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-text-secondary mt-0.5">
            Track your metrics, uploads, and video settings.
          </p>
        </div>
        <Button
          onClick={() => navigate("/upload")}
          className="flex items-center gap-1.5 self-start rounded-full font-bold"
        >
          <Upload size={14} />
        ...
        <div className="bg-bg-secondary/45 border border-border/50 rounded-xl p-5 flex items-center justify-between shadow-sm">
          <div className="space-y-1.5">
            ...
            <h3 className="text-2xl font-black text-text-primary">
              {formatViews(stats?.totalViews || 0).replace(" views", "")}
            </h3>
          </div>
            ...
            <table className="w-full text-left border-collapse text-xs">
              ...
              <tbody className="divide-y divide-border/30">
                {videos.map((vid) => (
                  <tr
                    key={vid._id}
                    className="hover:bg-bg-secondary/35 transition-colors"
                  >
                    ...
                    <td className="p-4">
                      ...
                      >
                        <span
                          className={`h-1.5 w-1.5 rounded-full ${vid.isPublished ? "bg-green-500 animate-pulse" : "bg-yellow-500"}`}
                        ></span>
          ...
          <form onSubmit={handleEditSubmit} className="space-y-4">
            ...
            <div className="space-y-1">
              ...
              <p className="text-[10px] text-text-muted">
                Upload a new image file to replace the current thumbnail.
              </p>
            </div>
